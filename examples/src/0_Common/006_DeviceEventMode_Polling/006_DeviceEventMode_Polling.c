...
BGAPI2_RESULT captureFrames(BGAPI2_Device* pDevice, BGAPI2_DataStream* pStream, BGAPI2_DeviceEvent* pEvent, int iEventCount) {
    ...
    BGAPI2_Node* pNode = NULL;
    char sEvent[1024] = "";
    bo_uint64  iEventlength = sizeof(sEvent);
    char sInterface[1024] = "";
    bo_uint64  iInterfaceLength = sizeof(sInterface);
    ...
    if (BGAPI2_Node_GetInterface(pNode, sInterface, &iInterfaceLength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
    ...
    for (framecount = 0; framecount < 10; framecount++) {
        ...
        for (iEventIndex = 0; iEventIndex < iEventCount; iEventIndex++) {
            ...
            if (res != BGAPI2_RESULT_SUCCESS) {
                ...
                iEventlength = sizeof(sEvent);
                if (BGAPI2_DeviceEvent_GetID(pEvent, sEvent, &iEventlength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
                printf("    Event.Id:            %s \n", sEvent);
                iEventlength = sizeof(sEvent);
                if (BGAPI2_DeviceEvent_GetName(pEvent, sEvent, &iEventlength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
                printf("    Event.Name:          %s \n", sEvent);
