...
// IMAGE
BGAPI2_RESULT handleImage(bo_uint64 iBufferFrameID, BGAPI2_Image* pImage, BGAPI2_ImageProcessor* pImgProc) {
...
BGAPI2_RESULT captureFrames(BGAPI2_DataStream* pStream, BGAPI2_ImageProcessor* pImgProc) {
    ...
    if (BGAPI2_ImageProcessor_CreateEmptyImage(pImgProc, &pImage) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
    ...
    for (framecount = 0; framecount < 8; framecount++) {
        ...
        if (res != BGAPI2_RESULT_SUCCESS) {
            ...
            if (bIsIncomplete == 1) {
                ...
                if (BGAPI2_Image_InitFromBuffer(pImage, pBufferFilled) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;

                if (handleImage(iFrameId, pImage, pImgProc) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
            }
            //QUEUE BUFFER
