...
BGAPI2_RESULT getFirstDevice(BGAPI2_Device** ppDevice) {
    ...
    printf("\niIndexTLProducer = %d", iNumTLProducer);
    for (iIndexTLProducer = 0; iIndexTLProducer < iNumTLProducer; iIndexTLProducer++) {
        ...
        printf("\niIndexInterface = %d", iNumInterfaces);
        for (iIndexInterface = 0; iIndexInterface < iNumInterfaces; iIndexInterface++) {
            ...
            if (iNumDevices > 0) {
                ...
                printf("\nBGAPI2_Device_Open:");
                res = BGAPI2_Device_Open(pDevice);
                printf("\nres = %d", res);
                if (res != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
                printf("\nprintDeviceInfo:");
...
BGAPI2_RESULT stopStreaming(BGAPI2_Device* pDevice, BGAPI2_DataStream* pStream) {
    ...
    // wait till the end of the current transmission
    while (is_grabbing) {
        BGAPI2_DataStream_GetIsGrabbing(pStream, &is_grabbing);
        waitms(10);
    }
