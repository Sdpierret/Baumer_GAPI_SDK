...
// CALLBACK DEVICE EVENTS
void BGAPI2CALL DevEventHandler(void * callbackOwner, BGAPI2_DeviceEvent * pDevEvent) {
    bo_uint64 iTimestamp = 0;
    char pString[1024] = "";           // defines an char array
    bo_uint64  iStringlength = sizeof(pString);
    BGAPI2_Device * pDevice = (BGAPI2_Device *)callbackOwner;

    printDeviceInfo(pDevice);
    printf("    Event received. \n");
    iStringlength = sizeof(pString);
    if (BGAPI2_DeviceEvent_GetID(pDevEvent, pString, &iStringlength) != BGAPI2_RESULT_SUCCESS) strcpy(pString, "ERROR");
    printf("    Event.Id:            %s \n", pString);
    iStringlength = sizeof(pString);
    if (BGAPI2_DeviceEvent_GetName(pDevEvent, pString, &iStringlength) != BGAPI2_RESULT_SUCCESS) strcpy(pString, "ERROR");
    printf("    Event.Name:          %s \n", pString);
    if (BGAPI2_DeviceEvent_GetTimeStamp(pDevEvent, &iTimestamp) != BGAPI2_RESULT_SUCCESS) strcpy(pString, "ERROR");
...
BGAPI2_RESULT captureFrames(BGAPI2_Device* pDevice, BGAPI2_DataStream* pStream, int iEventCount) {
    int framecount = 0;
    BGAPI2_Node* pNode = NULL;
    char pString[1024] = "";
    bo_uint64  iStringlength = sizeof(pString);
    ...
    if (BGAPI2_Node_GetInterface(pNode, pString, &iStringlength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
    ...
    *pEventsRegistered = 0;
    ...
    for (iNodeIndex = 0; iNodeIndex < iNodeCount; iNodeIndex++) {
        ...
        if (bReadable) {
            iStringlength = sizeof(pString);
            if (BGAPI2_Node_GetValue(pNode, pString, &iStringlength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
            printf("                             %2lld:  %-28s ", (long long)iNodeIndex, pString);
            if (BGAPI2_Node_SetString(pSelectorNode, pString) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
            if (BGAPI2_Device_GetRemoteNode(pDevice, "EventNotification", &pNode) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
            iStringlength = sizeof(pString);
            if (BGAPI2_Node_GetValue(pNode, pString, &iStringlength) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
            printf("%s\n", pString);
...
BGAPI2_RESULT enableDeviceEvents(BGAPI2_Device* pDevice) {
    printf("REGISTER CALLBACK FUNCTION\n");
    if (BGAPI2_Device_SetDeviceEventMode(pDevice, EVENTMODE_EVENT_HANDLER) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
    if (BGAPI2_Device_RegisterDeviceEventHandler(pDevice, pDevice, &DevEventHandler) != BGAPI2_RESULT_SUCCESS) return BGAPI2_RESULT_ERROR;
    return BGAPI2_RESULT_SUCCESS;
}

BGAPI2_RESULT disableDeviceEvents(BGAPI2_Device* pDevice) {
...
BGAPI2_RESULT run() {
    ...
    if (getFirstDevice(&pDevice) == BGAPI2_RESULT_SUCCESS) {
        res = enableDeviceEvents(pDevice);
        if (res == BGAPI2_RESULT_SUCCESS) {
        ...
        if (res == BGAPI2_RESULT_SUCCESS) {
            res = captureFrames(pDevice, pStream, iEventCount);
        }
        if (res == BGAPI2_RESULT_SUCCESS) {
            res = stopStreaming(pDevice, pStream);
        }
        if (res == BGAPI2_RESULT_SUCCESS) {
            res = disableDeviceEvents(pDevice);
        }
...
int main() {
    ...
    printf("Input any number to close the program:");
